  ...
  <div class="category-manage-container">
    ...
    <div class="tree-section">
      <el-table
        ...
        <el-table-column prop="weight" label="排序" width="100"></el-table-column>
    ...
    <el-dialog title="分类详情" v-model="viewDialogVisible" width="500px">
      <div class="category-detail" v-if="viewCategoryData">
        ...
        <div class="detail-row"><span class="label">分类级别：</span><span class="value"><el-tag :type="viewCategoryData.parentId == 1 ? 'primary' : 'success'">{{ viewCategoryData.parentId == 0 ? '一级分类' : '二级分类' }}</el-tag></span></div>
        <div class="detail-row" v-if="viewCategoryData.parentId != 0"><span class="label">父分类：</span><span class="value">{{ getParentCategoryName(viewCategoryData.parentId) }}</span></div>
        <div class="detail-row"><span class="label">排序序号：</span><span class="value">{{ viewCategoryData.weight }}</span></div>
...
const deleteCategory = async (category) => {
  try {
    ...
    const response = await request.post(`/categories/removeById?id=${category.id}`)
    debugger
    ...
    if (error !== 'cancel') ElMessage.error('此分类包含子分类，请先删除子分类')
  }
}
...
const saveCategory = async () => {
  try {
    ...
    const url = isEdit.value ? `/categories/updateById` : '/categories/save'
    const method = isEdit.value ? 'post' : 'post'
    categoryForm.createTime = categoryForm.createTime.replace("T", " ")
    categoryForm.updateTime = categoryForm.updateTime.replace("T", " ")
