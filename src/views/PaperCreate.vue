...
const fetchAllQuestions = async (callback) => {
  ...
  try {
    ...
    const res = await request.get('/questions/page', { params })
...
const handleManualSubmit = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      ...
      await request.post('/paper/createPaper', paperData)
...
const handleAiSubmit = async () => {
  ...
  try {
    await request.post('/paper/aiCreatePaper', paperData)
...
const loadPaperDataForEdit = async () => {
  try {
    const res = await request.get(`/paper/getById?id=${paperId.value}`)
