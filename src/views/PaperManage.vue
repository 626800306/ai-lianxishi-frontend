...
const updateStatus = async (paper, status) => {
  try {
    await request.post(`/paper/updateStatus?id=${paper.id}&status=${status}`)
...
const deletePaper = async (paper) => {
  try {
    ...
    await request.post(`/paper/removeById?id=${paper.id}`)
...
const handleBatchDelete = async () => {
  ...
  try {
    ...
    const  idList = selectedPapers.value.map(p => p.id)
