                        ...
                        <span :class="{ 'correct-choice': choice.isCorrect }">
                          {{ String.fromCharCode(65 + index) }}. {{ choice.content }}
                                ...
                                <el-table-column label="操作" width="100" fixed="right">
                                    <template #default="{ row, $index }">
                                        <el-button size="small" type="primary" @click="editAiQuestion($index)"
                                                   icon="Edit">编辑
                                        </el-button>
                                    </template>
                                </el-table-column>
...
<script setup>
    ...
    const fetchQuestions = async () => {
        loading.value = true
        try {
            const res = await request.get('questions/page', {
                params: {
                    ...searchForm,
    ...
    /*const downloadTemplate = async () => {
      try {
        debugger
        const response = await fetch('/questions/downloadTemplate', {
          method: "GET"
        })
    ...
    const downloadTemplate = async () => {
      ...
      const url = request.defaults.baseURL + `/questions/downloadTemplate?t=${timestamp}`
      ...
      // 提示用户
      ElMessage.success('开始下载模板，请检查浏览器下载列表')
    ...
    */
    ...
    const previewExcelData = async () => {
      ...
      try {
        ...
        const res = await request.post('/questions/previewExcel', formData, {
          headers: {
    ...
    const confirmImport = async () => {
        ...
        try {
            const res = await request.post('/questions/importQuestions', previewData.value)
    ...
    const startAiGenerate = async () => {
        ...
        try {
            ...
            const res = await request.post('/questions/kimiAiCreateQuestions', requestData, {
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (isEdit.value) {
                await request.post(`/questions/updateById`, questionForm)
                ElMessage.success('题目更新成功')
            } else {
                // questionForm.categoryId = parseInt(questionForm.categoryId)
                await request.post('/questions/save', questionForm)
    ...
    const handleDelete = async (row) => {
        try {
            ...
            await request.post(`/questions/removeById?id=${row.id}`)
    ...
    const handleBatchDelete = async () => {
        ...
        try {
            ...
            // const deletePromises = request.post("/questions/removeBatchByIds", {ids: selectedQuestions.value})
            const deletePromises = request.post("/questions/removeBatchByIds", selectedQuestions.value)
