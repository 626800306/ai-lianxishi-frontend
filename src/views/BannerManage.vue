...
const getBannerList = async () => {
  loading.value = true
  try {
    const res = await request.get('/banners/page-select-all')
...
const submitBanner = async () => {
  ...
  try {
    ...
    const url = isEdit ? '/banners/update' : '/banners/save'
...
const toggleBannerStatus = async (banner) => {
  try {
    ...
    await request.post(`/banners/switch-status?id=${banner.id}`)
...
const deleteBanner = async (banner) => {
  try {
    ...
    await request.post(`banners/removeById?id=${banner.id}`)
